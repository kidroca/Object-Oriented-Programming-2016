 ...
 *  manufacturer, price, owner, battery
 * characteristics (model, hours idle and hours talk) and display characteristics 
 * (size and number of colors).
 * Define 3 separate classes (class GSM holding instances of the classes Battery and Display). */

/***************** Problem 2. Constructors *****************/
/* Define several constructors for the defined classes that take 
 * different sets of arguments (the full information for the
 * class or part of it).
...
/* Add an enumeration BatteryType (Li-Ion, NiMH, NiCd, …) and use 
...
/* Use properties to encapsulate the data fields inside the GSM, 
 * Battery and Display classes.
 ...
 *      Assuming that the price per minute is 0.37 calculate and print the 
 *          total price of the calls in the history.
...
namespace MobileDevices
{
    ...
    class StartUp
    {
        static void Main()
        {
            Console.WriteLine("Choose a test: 'main' or 'calls'");
            string userChoice;
            ...
            do
            {
                ...
                else if (userChoice == "calls")
                {
                    GSMCallHistoryTest.CallTests();
                }
                else if (userChoice != "exit")
                {
                    Console.WriteLine("Try otnovo");
                }
        ...
        static void PrintProductProperties(object product)
        {
            foreach (var prop in product.GetType().GetProperties())
            {
                Console.WriteLine("{0}: {1}", prop.Name, prop.GetValue(product, null));
            }
        }
    }
}
